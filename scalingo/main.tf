...
resource "scalingo_app" "marinaperez-my-app-python-api" {
...
resource "scalingo_addon" "db" {
  ...
  app = "${scalingo_app.marinaperez-my-app-python-api.id}"
}

resource "scalingo_container_type" "web" {
  app    = scalingo_app.marinaperez-my-app-python-api.name
  name   = "web"
  amount = 1
  size   = "S"
}
